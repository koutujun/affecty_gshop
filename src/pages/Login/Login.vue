        ...
        <form @submit.prevent="login">
          <div :class="{on: loginWay}">
            <section class="login_message">
              ...
              <button :disabled="!validatePhone" class="get_verification" :class="{right_phone: validatePhone}" @click.prevent="sendCode">
                {{this.computedTime>0?`已发送(${this.computedTime}s)`:'获取验证码'}}
              </button>
            </section>
      ...
      async sendCode(){
        ...
        if(this.computedTime===0){
          ...
          this.inter = setInterval(()=>{
            this.computedTime--
            if(this.computedTime <= 0){
              clearInterval(this.inter)  //清除循环定时器
            }
          },1000)
        }
